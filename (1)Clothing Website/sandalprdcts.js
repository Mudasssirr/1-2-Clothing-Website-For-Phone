...
document.addEventListener('DOMContentLoaded', () => {
    const productDetails = document.getElementById('Sandal-product-details');
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('id');

    // Find the product with the matching ID
    const product = sandalproducts.find(p => p.id.toString() === productId);
    ...
    if (product) {
        ...
        <p class='product-cut'>Size: <span class='light-font'> ${product.defsize} </span></p>
        </div>
